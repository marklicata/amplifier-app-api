"""Provider configuration management for Amplifier applications.

This module provides high-level provider management that orchestrates:
- Provider configuration at different scopes (local/project/global)
- Provider discovery from entry points and sources
- Current provider inspection
- Provider reset operations

This is a key component that application developers should use rather than
directly manipulating config files.
"""
...
class ProviderManager:
    ...
    This is the primary interface for provider management in Amplifier applications.
    It handles provider configuration, discovery, and inspection with scope-aware
    resolution (local > project > global > profile).
    ...
    Example:
        ...
        print(f"Using {current.module_id} from {current.source}")
        ...
        for module in modules:
            if module.type == "provider":
                ...
                if info and "display_name" in info:
                    ...
                    logger.debug(f"Fallback to module name for display_name: {module.name}")
    ...
    def _discover_providers_from_sources(self) -> dict[str, tuple[str, str, str]]:
        ...
        for module_id, source_uri in effective_sources.items():
            try:
                ...
                logger.debug(f"Attempting to get provider info for {module_id}")
                # Try to get provider info via direct import
                info = get_provider_info(module_id)
                logger.debug(f"Provider info for {module_id}: {info}")
                if info:
